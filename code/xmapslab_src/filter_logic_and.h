...
#ifndef FILTER_LOGIC_AND
#define FILTER_LOGIC_AND
...
#define DEFINED_FILTER_LOGIC_AND

namespace _f_logic_and_ns
{
  // parameter 1
...
//  const int Parameter1_divisor=10;
  ...
  const std::string FILTER_NAME="Logic AND";
}

class _window_xmapslab;
class _qtw_filter_logic_and;

/*************************************************************************/

class _filter_logic_and : public _filter
{
public:
  _filter_logic_and();
  void reset_data();
...
class _filter_logic_and_ui : public _filter_logic_and
{
public:
  _filter_logic_and_ui(_window_xmapslab *Window1, std::string Name1=_f_logic_and_ns::FILTER_NAME);
  _filter_logic_and_ui(_window_xmapslab *Window1,std::map<std::string,std::string> &Parameters,std::string Name1=_f_logic_and_ns::FILTER_NAME);
  ~_filter_logic_and_ui();
  void reset_data();
...
//  void color(QColor Color1);
  ...
  void parameter1(int Value){_filter_logic_and::threshold(Value);}
  int parameter1(){return _filter_logic_and::threshold();}

//  void parameter2(QColor Color1){_filter_logic_and::color(Color1);}
//  QColor parameter2(){return _filter_logic_and::color();}

private:
  _qtw_filter_logic_and *Qtw_filter_logic_and=nullptr;
};

/*************************************************************************/

class _qtw_filter_logic_and: public QWidget
{
  Q_OBJECT
public:

  _qtw_filter_logic_and(_window_xmapslab *Window1,_filter_logic_and_ui *Filter1,std::string Box_name=_f_logic_and_ns::FILTER_NAME);
...
//  _button_color_simple *Button_parameter2=nullptr;
  ...
  _filter_logic_and_ui *Filter=nullptr;
