...
_filter_combination_addition::_filter_combination_addition()
{
  Filter_type=_filter_ns::_filter_type::FILTER_TYPE_COMBINATION_ADDITION_OP;
  Filter_class=_filter_ns::_filter_class::FILTER_CLASS_COMBINATION;
}

/*************************************************************************/

void _filter_combination_addition::reset_data()
{
}

/*************************************************************************/

float _filter_combination_addition::get_max_value()
{
...
void _filter_combination_addition::update()
{
  float Value0=0;
  float Value1=0;
  ...
  float Threshold_value1=Max_input_value1*Threshold1;
  ...
  if (Logic!=nullptr){
    for (int Row=0;Row<Input_A->rows;Row++){
      for (int Col=0;Col<Input_A->cols;Col++){
        if (Logic->at<unsigned char>(Row,Col)==255){
  ...
  if (Relative_normalization==true){
    ...
    Normalization_value=(Max_input_value0+Max_input_value1)/2.0f;
  }

  for (int Row=0;Row<Vec_input_data[1]->rows;Row++){
    for (int Col=0;Col<Vec_input_data[1]->cols;Col++){
      Vec_output_data[0]->at<unsigned char>(Row,Col)=static_cast<unsigned char>(Output_aux->at<float>(Row,Col)*255.0f/Normalization_value);
    }
  }
...
void _filter_combination_addition::color(QColor Color1)
{
  Color=Color1;
...
_filter_combination_addition_ui::_filter_combination_addition_ui(_window_xmapslab *Window1, std::string Name1)
{
  Name=Name1;
  Window=Window1;
  Qtw_filter_combination_addition=new _qtw_filter_combination_addition(Window1,this,Name1);
}

/*************************************************************************/

_filter_combination_addition_ui::_filter_combination_addition_ui(_window_xmapslab *Window1, std::map<string, string> &Parameters, std::string Name1)
{
...
_filter_combination_addition_ui::~_filter_combination_addition_ui()
{
  delete Qtw_filter_combination_addition;
}

/*************************************************************************/

void _filter_combination_addition_ui::reset_data()
{
  _filter_combination_addition::reset_data();
  Qtw_filter_combination_addition->parameter1(parameter1());
  hide();
}

/*************************************************************************/

void _filter_combination_addition_ui::show()
{
  Qtw_filter_combination_addition->show();
}

/*************************************************************************/

void _filter_combination_addition_ui::hide()
{
  Qtw_filter_combination_addition->hide();
}

/*************************************************************************/

void *_filter_combination_addition_ui::get_link()
{
  return Qtw_filter_combination_addition->get_link();
}

/*************************************************************************/

void _filter_combination_addition_ui::read_parameters(std::map<std::string,std::string> &Parameters)
{
  if (Parameters["_INI_"]=="EDITOR"){// default parameters
    parameter1(_f_combination_addition_ns::THRESHOLD_DEFAULT);
  }
  else{// Parameters from file or from initialised filter
    try{
      if (Parameters["threshold"]=="default") parameter1(_f_combination_addition_ns::THRESHOLD_DEFAULT);
      else parameter1(atoi(Parameters["threshold"].c_str()));
    }
    catch (...) {
      QMessageBox MsgBox;
      MsgBox.setText("Error in the data of combination addition filter");
      MsgBox.exec();
      exit(-1);
    }
  }
...
void _filter_combination_addition_ui::write_parameters(std::map<std::string,std::string> &Parameters)
{
  char Aux[100];
...
void _filter_combination_addition_ui::color(QColor Color1)
{
  Color=Color1;
  Qtw_filter_combination_addition->parameter2(Color);
}

/*************************************************************************/

_qtw_filter_combination_addition::_qtw_filter_combination_addition(_window_xmapslab *Window1,_filter_combination_addition_ui *Filter1,std::string Box_name)
{
  Window=Window1;
  Filter=Filter1;
...
void _qtw_filter_combination_addition::parameter1(int Value)
{
...
void _qtw_filter_combination_addition::parameter2(QColor Color1)
{
...
void _qtw_filter_combination_addition::parameter1_slot(int Value)
{
...
void _qtw_filter_combination_addition::parameter2_slot(QColor Color1)
{
