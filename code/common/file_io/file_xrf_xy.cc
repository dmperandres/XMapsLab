...
//bool _file_xrf_xy::read(std::vector<float> &Vec_data,float &Step,std::string &Error)
//{
//  float Value,Value1;
//  std::string Text;
//  std::string Token;
...
bool _file_xrf_xy::read(std::vector<float> &Vec_data,float &Step,std::string &Error)
{
  ...
  try{
    ...
    while (Found==false && get_new_line(File,Token)){
      ...
      Aux=QString::fromStdString(Token);
      Aux.replace(Regex, ";");
      Token=Aux.toStdString();

      get_tokens(Token,';',Tokens);
      Text=Tokens[0];
      replace(Text.begin(),Text.end(),',','.');
      Position++;
      Previous_pos_kev=stof(Text)*Step;
...
//void _file_xrf_xy::write(std::vector<float> &Vec_data)
//{
