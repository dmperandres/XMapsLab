...
bool _data_io::load_project_data_csv(QWidget *Window, std::string File_name, _project_data_ns::_project_data &Project_data)
{
  // read the positions
  std::string Error;
  ...
  if (File.open(File_name,_file_project_data_csv::_mode::MODE_READ)){
    ...
    QMessageBox::warning(Window, Window->tr("Warning"),QString::fromStdString("The file "+File_name+" cannot be opened"));
    return false;
  }
}
...
// bool _data_io::xrf_load_interpreted_csv(QWidget *Window, std::string File_name, std::vector<_project_data_ns::_measured_data> &Vec_sampled_data, std::vector<std::string> &Vec_names_pdb)
// {
//   std::string Error;
//   _file_measured_csv File;
...
//   if (Project_data.Width_pixel>0 && Project_data.Height_pixel>0){
//     std::string Error;
...
bool _data_io::load_colormaps_csv(QWidget *Window, std::string Dir, std::map<std::string,std::vector<_vertex3f>> &Map_fixed_palettes)
{
  ...
  if (Files_csv.size()>0){
    foreach(QString File_name, Files_csv) {
      ...
      if(File.open(Long_name,_file_io::_mode::MODE_READ)){
