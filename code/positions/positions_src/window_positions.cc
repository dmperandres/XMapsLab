...
void _window_positions::load_auxiliary_image_slot()
{
  ...
  if (Dialog.exec()==QDialog::Accepted && File_auxiliary_image!=""){
    ...
    Auxiliary_gl_widget->update_size_texture(Auxiliary_image->cols,Auxiliary_image->rows);
...
void _window_positions::load_positions()
{
  ...
  // convert to pixels
  // ¡¡¡¡ Aviso: en positions las marcas de las posiciones son una textura OpenGL y en XMapsLab las crea QPainter !!!

  // add the positions
  Vec_positions.resize(Project_data.Vec_coordinates_x.size());
  int Pos_x;
  int Pos_y;
  for (unsigned int i=0;i<Project_data.Vec_coordinates_x.size();i++){
    Pos_x=int(roundf(Project_data.Vec_coordinates_x[i]*Project_data.Width_pixel));
    Pos_y=int(roundf(Project_data.Vec_coordinates_y[i]*Project_data.Height_pixel));
    Vec_positions[i]={int(i)+1,_vertex2i(Pos_x,Pos_y),false};
  }
...
//  Project_data.Num_samples=Vec_positions.size();
  ...
  //   Project_data.Vec_coordinate_x[i]=float(Vec_positions[i].Coordinates.x)/float(Initial_width);
  //   Project_data.Vec_coordinate_y[i]=float(Vec_positions[i].Coordinates.y)/float(Initial_height);
  // }
...
void _window_positions::save_image(QString File_name)
{
  ...
  static_cast<_layer_positions_positions *>(Vec_layers[1].get())->draw(Painter,Width1,Height1, display_font_size()*print_font_factor(), display_out_circle_size()*print_font_factor(), display_font_color(), display_out_circle_color(), display_in_circle_color());

  Painter.end();

  Composed_image.save(File_name,"PNG");
}
...
void _window_positions::save_image_positions()
{
  ...
  if (QDir(folderPath).exists()){
    QFileDialog Dialog(this,"Save image",Project_dir+"/results","*.png");
    Dialog.setFileMode(QFileDialog::AnyFile);
    Dialog.setAcceptMode(QFileDialog::AcceptSave);
    Dialog.setDefaultSuffix("png");
    Dialog.selectFile("positions.png");
    Dialog.setWindowTitle("Select a file name");
    if (Dialog.exec()== QDialog::Accepted){
      QString File_name=Dialog.selectedFiles().at(0);

      save_image(File_name);

      QMessageBox::information(this,tr("Information"),tr("The image with positions has been correctly saved"));
    }
  }
  else{
...
void _window_positions::add_layer_image(std::shared_ptr<cv::Mat> Image1)
{
  ...
  Main_gl_widget->update_size_texture(Main_image->cols,Main_image->rows);
...
void _window_positions::add_layer_positions()
{
  ...
  Layer->parameters( display_font_size(), display_out_circle_size(), display_font_color(), display_out_circle_color(), display_in_circle_color(), display_selection_out_circle_color(), display_selection_in_circle_color());
  Layer->update();
...
void _window_positions::parameters_changed()
{
  if (Loaded_image==true){
    std::static_pointer_cast<_layer_positions_positions_ui>(Vec_layers[1])->parameters(display_font_size(), display_out_circle_size(), display_font_color(), display_out_circle_color(), display_in_circle_color(), display_selection_out_circle_color(), display_selection_in_circle_color());
    std::static_pointer_cast<_layer_positions_positions_ui>(Vec_layers[1])->update();
...
{
  display_font_size(Font_size_display1);
  display_out_circle_size(Out_circle_size_display1);
  display_font_color(Font_color_display1);
  display_out_circle_color(Out_circle_color_display1);
  display_in_circle_color(In_circle_color_display1);
  display_selection_out_circle_color(Selection_out_circle_color_display1);
  display_selection_in_circle_color(Selection_in_circle_color_display1);
  // Copy_display_values=Copy_display_values1;

  // if (Copy_display_values==true){
  //   print_font_size(Font_size_display1);
  //   print_out_circle_size(Out_circle_size_display1);
  //   print_font_color(Font_color_display1);
  //   print_out_circle_color(Out_circle_color_display1);
  //   print_in_circle_color(In_circle_color_display1);
  // }
...
// {
//   print_font_size(Font_size_print1);
//   print_out_circle_size(Out_circle_size_print1);
//   print_font_color(Font_color_print1);
//   print_out_circle_color(Out_circle_color_print1);
//   print_in_circle_color(In_circle_color_print1);
