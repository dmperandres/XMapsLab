...
void _gl_widget_positions::update_size_texture(int Width1,int Height1)
{
  makeCurrent();
...
//    int Step = Num_degrees.y() / 120;
    ...
    if (Event->angleDelta().y()<0){
      Scale_factor/=_common_ns::MOUSE_SCALING_FACTOR;
      Factor=1/_common_ns::MOUSE_SCALING_FACTOR;
    }
    else{
      Scale_factor*=_common_ns::MOUSE_SCALING_FACTOR;
      Factor=_common_ns::MOUSE_SCALING_FACTOR;
    }
...
void _gl_widget_positions::draw_objects()
{
  ...
  if (Window->Vec_layers.size()>0){
    ...
    int Width=int(roundf(float(Window_width)*Scale_factor));
    int Height=int(roundf(float(Window_height)*Scale_factor));
