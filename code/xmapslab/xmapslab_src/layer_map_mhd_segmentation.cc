...
_layer_map_mhd_segmentation::_layer_map_mhd_segmentation()
{
  Type=_layer_xmapslab_ns::_type::TYPE_MAP;
  Subtype=_layer_xmapslab_ns::_subtype::SUBTYPE_MHD_SEGMENTATION;
  ...
  #include "mhd_segmentation.vert"
  };

  static const GLchar * P0_fs[]={
  #include "mhd_segmentation.frag"
  };

  Program0=Shader.load_shaders(P0_vs,P0_fs);
  if (Program0==0){
    QMessageBox::critical(Window, Window->tr("Error"),Window->tr("Error with the GPU program 'mhd_segmentation'"));
    QApplication::quit();
    exit(-1);
  }
}
...
void _layer_map_mhd_segmentation::create_buffers()
{
  ...
  for (unsigned int i=0;i<Vec_element_data.size();i++){
    ...
    Vec_element_data[i].Color=Image_normalized->at<cv::Vec4f>(Vec_element_data[i].y*Height,Vec_element_data[i].x*Width);
    // std::cout << "pos=" << i << " x=" << Vec_element_data[i].x*Width << " y=" << Vec_element_data[i].y*Height << " Value=" << Vec_element_data[i].Value << " ID=" << Vec_element_data[i].Mask_id <<" color=" << Vec_element_data[i].Color << std::endl;
  }
  ...
  // The buffer for the positions data (x, y, value, segment_id, color).It will be a SSB
  if (Positions_data>0) glDeleteBuffers(1,&Positions_data);
...
void _layer_map_mhd_segmentation::update_map()
{
  ...
  glBindImageTexture(2,Tex_image_result,0,GL_FALSE,0,GL_READ_WRITE,GL_R32F);
...
void _layer_map_mhd_segmentation::update()
{
  update_map();
}
...
// float _layer_map_mhd_segmentation::get_value(int Col,int Row)
// {
...
_layer_map_mhd_segmentation_ui::_layer_map_mhd_segmentation_ui(_window_xmapslab *Window1, std::string Name1)
{
  Window=Window1;
  Name=Name1;
  Qtw_layer_map_mhd_segmentation_a=new _qtw_layer_map_mhd_segmentation(Window1,this,Name);
}

//HEA

_layer_map_mhd_segmentation_ui::~_layer_map_mhd_segmentation_ui()
{
  delete Qtw_layer_map_mhd_segmentation_a;
}

//HEA

void _layer_map_mhd_segmentation_ui::reset_data()
{
  _layer_map_mhd_segmentation::reset_data();
  Qtw_layer_map_mhd_segmentation_a->parameter1(parameter1());
  hide();
}

//HEA

void _layer_map_mhd_segmentation_ui::show()
{
  Qtw_layer_map_mhd_segmentation_a->show();
}

//HEA

void _layer_map_mhd_segmentation_ui::hide()
{
  Qtw_layer_map_mhd_segmentation_a->hide();
}

//HEA

void *_layer_map_mhd_segmentation_ui::get_link()
{
  return Qtw_layer_map_mhd_segmentation_a->get_link();
}

//HEA

void _layer_map_mhd_segmentation_ui::enable()
{
  Qtw_layer_map_mhd_segmentation_a->enable();
}

//HEA

void _layer_map_mhd_segmentation_ui::disable()
{
  Qtw_layer_map_mhd_segmentation_a->disable();
}

//HEA

void _layer_map_mhd_segmentation_ui::parameter1_ui(float Value)
{
  Qtw_layer_map_mhd_segmentation_a->parameter1(int(Value*255.0f));
}

//HEA

void _layer_map_mhd_segmentation_ui::parameter2_ui(float Value)
{
  Qtw_layer_map_mhd_segmentation_a->parameter2(int(Value*255.0f));
}

//HEA

void _layer_map_mhd_segmentation_ui::parameter3_ui(float Value)
{
  Qtw_layer_map_mhd_segmentation_a->parameter3(int(Value*255.0f));
}

//HEA

void _layer_map_mhd_segmentation_ui::parameter4_ui(int Value)
{
  Qtw_layer_map_mhd_segmentation_a->parameter4(Value);
}

//HEA

void _layer_map_mhd_segmentation_ui::data_type_print(_palette_data_ns::_data_type_print Data_type_print1)
{
  Data_type_print=Data_type_print1;
  Qtw_layer_map_mhd_segmentation_a->parameter4(int(Data_type_print1));
}

//HEA

_qtw_layer_map_mhd_segmentation::_qtw_layer_map_mhd_segmentation(_window_xmapslab *Window1,_layer_map_mhd_segmentation_ui *Filter1,std::string Box_name)
{
  Window=Window1;
  Filter=Filter1;
...
void _qtw_layer_map_mhd_segmentation::parameter1(int Value)
{
...
void _qtw_layer_map_mhd_segmentation::parameter2(int Value)
{
...
void _qtw_layer_map_mhd_segmentation::parameter3(int Value)
{
...
void _qtw_layer_map_mhd_segmentation::parameter4(int Value)
{
...
void _qtw_layer_map_mhd_segmentation::parameter1_slot(int Value)
{
...
void _qtw_layer_map_mhd_segmentation::parameter2_slot(int Value)
{
...
void _qtw_layer_map_mhd_segmentation::parameter3_slot(int Value)
{
...
void _qtw_layer_map_mhd_segmentation::parameter4_slot(int Value)
{
