...
#ifndef LAYER_MAP_VALUE_SEGMENTATION_H
#define LAYER_MAP_VALUE_SEGMENTATION_H
...
#define DEFINED_LAYER_MAP_VALUE_SEGMENTATION

namespace _layer_map_value_segmentation_ns
{
//   // parameter 1
...
//   const _position_normalization_type NORMALIZATION_TYPE_DEFAULT=_position_normalization_type::POSITION_NORMALIZATION_HOMOGENEOUS;
  ...
  enum class _computation_mode:unsigned char {COMPUTATION_MODE_MEAN,COMPUTATION_MODE_MINIMUM,COMPUTATION_MODE_MAXIMUM};
  const _computation_mode COMPUTATION_MODE_DEFAULT=_computation_mode::COMPUTATION_MODE_MEAN;
}

// class _window;
class _qtw_layer_map_value_segmentation;

//HEA

class _layer_map_value_segmentation: public _layer_map
{
public:
  _layer_map_value_segmentation();
  ~_layer_map_value_segmentation();

  void add_masks_id_image(std::shared_ptr<cv::Mat> Image1){Masks_ids_image=Image1;}

  void computation_mode(int Computation_mode1){Computation_mode=Computation_mode1;}
  ...
  // void position_normalization_type(_layer_map_value_segmentation_ns::_position_normalization_type Position_normalization_type1){Position_normalization_type=Position_normalization_type1;}
  // the value that represents a no-mask
  void null_value(int Null_value1){Null_value=Null_value1;}
...
protected:
  ...
  // _layer_map_value_segmentation_ns::_position_normalization_type Position_normalization_type=_layer_map_value_segmentation_ns::NORMALIZATION_TYPE_DEFAULT;
  // int Probe=0;
  _palette_data_ns::_data_type_print Data_type_print;
  //
  std::shared_ptr<cv::Mat> Masks_ids_image=nullptr;
  GLuint Tex_input_masks_ids=-1;
  GLuint Positions_data=-1;
  int Null_value;
  //
...
class _layer_map_value_segmentation_ui : public _layer_map_value_segmentation
{
public:
  _layer_map_value_segmentation_ui(_window_xmapslab *Window1, std::string Name1=_layer_map_value_segmentation_ns::BOX_TEXT);
  ~_layer_map_value_segmentation_ui();
  void reset_data();
...
//  void get_parameters(std::map<std::string,std::string> &Parameters);
  ...
  void parameter1(int Value){_layer_map_value_segmentation::transparency(float(Value)/255.0f);}
  void parameter1_ui(float Value);
  int parameter1(){return int(_layer_map_value_segmentation::transparency()*255.0f);}

  void parameter2(int Value){_layer_map_value_segmentation::color_mixing_min(float(Value)/255.0f);}
  void parameter2_ui(float Value);
  int parameter2(){return int(_layer_map_value_segmentation::color_mixing_min()*255.0f);}

  void parameter3(int Value){_layer_map_value_segmentation::color_mixing_max(float(Value)/255.0f);}
  void parameter3_ui(float Value);
  int parameter3(){return int(_layer_map_value_segmentation::color_mixing_max()*255.0f);}

  void parameter4(int Value){_layer_map_value_segmentation::data_type_print(_palette_data_ns::_data_type_print(Value));}
  void parameter4_ui(int Value);
  int parameter4(){return int(_layer_map_value_segmentation::data_type_print());}
...
private:
  _qtw_layer_map_value_segmentation *Qtw_layer_map_value_segmentation_a=nullptr;
};

//HEA

class _qtw_layer_map_value_segmentation: public QWidget
{
  Q_OBJECT
public:
  _qtw_layer_map_value_segmentation(_window_xmapslab *Window1,_layer_map_value_segmentation_ui *Filter1,std::string Box_name="Canny parameters");
...
//  QLineEdit *Line_edit_parameter1=nullptr;
  ...
  _layer_map_value_segmentation_ui *Filter=nullptr;
