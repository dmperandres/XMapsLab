...
//  QString Str;
  ...
  Combobox_lab_palette_color_assigning->addItem("Random");
  ...
  //
  QLabel *Label_lab_palette_fixed_color=new QLabel("Color");

  // colors
  _button_color_simple *Button_lab_palette_fixed_color=new _button_color_simple(LAB_palette_fixed_color,false);
  connect(Button_lab_palette_fixed_color,SIGNAL(colorChanged(QColor)),this,SLOT(lab_changed_palette_fixed_color_slot(QColor)));
  ...
  // Palette fixed color 4
  Grid_lab_palette_parameters->addWidget(Label_lab_palette_fixed_color,LAB_palette_parameters_num_rows,0,Qt::AlignRight);
  Grid_lab_palette_parameters->addWidget(Button_lab_palette_fixed_color,LAB_palette_parameters_num_rows++,1,Qt::AlignLeft);
...
void _nodes_editor_maps_tab::add_map_to_layers_slot()
{
  ...
  if (Window->Map_filters[Selected_image]->filter_type()!=_filter_ns::_filter_type::FILTER_TYPE_ELEMENT){
    ...
    _palette_data_ns::_palette Palette={LAB_palette_type, LAB_palette_color_type, 0, LAB_palette_color_assigning, LAB_palette_fixed_color, LAB_palette_name_fixed_palette.toStdString()};
    ...
    Window->add_layer_map_lab(Name, Window->Map_filters[Selected_image]->get_result(), Palette, Data_type_print, Max_value, Window->zero_color());
...
void _nodes_editor_maps_tab::lab_changed_palette_color_type_slot(int Index)
{
  ...
  if (LAB_palette_color_type==_palette_data_ns::_palette_color_type::PALETTE_COLOR_TYPE_SINGLE_TONE){
    if (LAB_palette_color_assigning==_palette_data_ns::_palette_color_assigning_type::PALETTE_COLOR_ASSIGNING_FIXED){
      ...
      change_row_visibility(Grid_lab_palette_parameters,6,false);
    }
    else{
    ...
    if (LAB_palette_color_assigning==_palette_data_ns::_palette_color_assigning_type::PALETTE_COLOR_ASSIGNING_FIXED){
      change_row_visibility(Grid_lab_palette_parameters,5,false);
      change_row_visibility(Grid_lab_palette_parameters,6,true);
    }
    else{
      change_row_visibility(Grid_lab_palette_parameters,5,false);
      change_row_visibility(Grid_lab_palette_parameters,6,false);
...
void _nodes_editor_maps_tab::lab_changed_palette_fixed_color_slot(QColor Color)
{
  LAB_palette_fixed_color=Color;
}
...
void _nodes_editor_maps_tab::lab_button_pushed_palette_parameters_slot(bool State)
{
  ...
  if (LAB_show_palette_parameters==true){
    ...
    if (LAB_palette_color_assigning==_palette_data_ns::_palette_color_assigning_type::PALETTE_COLOR_ASSIGNING_RANDOM){
