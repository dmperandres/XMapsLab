      ...
      for (int Col=0;Col<Input_A->cols;Col++){
        if (Logic->at<float>(Row,Col)>0.5){ // True
          if (Input_A->at<float>(Row,Col)>Threshold1){
            Output->at<float>(Row,Col)=Input_A->at<float>(Row,Col)*Factor;
            if (Output->at<float>(Row,Col)>Max_value0) Max_value0=Output->at<float>(Row,Col);
          }
  ...
  if (Max_value0>0){
    for (int Row=0;Row<Vec_input_data[1]->rows;Row++){
      for (int Col=0;Col<Vec_input_data[1]->cols;Col++){
        Output->at<float>(Row,Col)=Output->at<float>(Row,Col)/Max_value0;
      }
    }
  }
