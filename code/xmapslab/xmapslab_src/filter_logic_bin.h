...
#ifndef FILTER_LOGIC_BIN
#define FILTER_LOGIC_BIN
...
#define DEFINED_FILTER_LOGIC_BIN

namespace _f_logic_bin_ns
{
  // parameter 1
...
//  const int Parameter1_divisor=10;
  ...
  const std::string FILTER_NAME="Logic BIN";
}

class _window;
class _qtw_filter_logic_bin;

//HEA

class _filter_logic_bin : public _filter
{
public:
  _filter_logic_bin();
  void reset_data();
...
class _filter_logic_bin_ui : public _filter_logic_bin
{
public:
  _filter_logic_bin_ui(_window_xmapslab *Window1, std::string Name1=_f_logic_bin_ns::FILTER_NAME);
  _filter_logic_bin_ui(_window_xmapslab *Window1,std::map<std::string,std::string> &Parameters,std::string Name1=_f_logic_bin_ns::FILTER_NAME);
  ~_filter_logic_bin_ui();
  void reset_data();
...
//  void color(QColor Color1);
  ...
  void parameter1(int Value){_filter_logic_bin::threshold(Value);}
  int parameter1(){return _filter_logic_bin::threshold();}

//  void parameter2(QColor Color1){_filter_logic_bin::color(Color1);}
//  QColor parameter2(){return _filter_logic_bin::color();}

private:
  _qtw_filter_logic_bin *Qtw_filter_logic_bin=nullptr;
};

//HEA

class _qtw_filter_logic_bin: public QWidget
{
  Q_OBJECT
public:

  _qtw_filter_logic_bin(_window_xmapslab *Window1,_filter_logic_bin_ui *Filter1,std::string Box_name=_f_logic_bin_ns::FILTER_NAME);
...
//  _button_color_simple *Button_parameter2=nullptr;
  ...
  _filter_logic_bin_ui *Filter=nullptr;
