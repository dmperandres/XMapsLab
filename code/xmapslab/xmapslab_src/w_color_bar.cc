...
void _w_color_bar::create_palettes()
{
  ...
  for (unsigned int i=0;i<Window->Vec_palette_data.size();i++){
    if (Window->Vec_palette_data[i].type()==_palette_data_ns::_palette_type::PALETTE_TYPE_DISCRETE){
      Palettes.push_back(new _color_bar_sections(Window->Vec_palette_data[i].color_type(),Window->Vec_palette_data[i].num_stops(),Window->Vec_palette_data[i].vec_values(),Window->Vec_palette_data[i].vec_colors(),Window->zero_color(),Window));
    }
    else{
...
//    }
  ...
  case _w_tab_xmapslab_ns::_tabs::DATA_TAB:
      // palettes
      Widget_color_bar_layers->hide();
...
_color_bar_abstract *_w_color_bar::create_color_bar(_palette_data_ns::_palette_data Palette1, std::string Name_element1, float Max_value, std::vector<QColor> &Colors, _palette_data_ns::_data_type_print Data_type_print1, _common_ns::_zero_color Zero_color1)
{
  ...
  if (Palette1.type() ==_palette_data_ns::_palette_type::PALETTE_TYPE_DISCRETE){    
    _color_bar_sections *Color_bar_sections=new _color_bar_sections(Palette1.color_type(),Palette1.num_stops(),Palette1.vec_values(),Colors,Zero_color1,Window);
...
//  cout << "change color" << endl;
  ...
  switch (Window->Map_name_to_layer[Name].Layer->subtype()){
  case _layer_xmapslab_ns::_subtype::SUBTYPE_MHD_DISTANCE:
    ...
    break;
  case _layer_xmapslab_ns::_subtype::SUBTYPE_TRIANGULATION:
  ...
  case _layer_xmapslab_ns::_subtype::SUBTYPE_COMPOUND:
    ...
    break;
#endif
  case _layer_xmapslab_ns::_subtype::SUBTYPE_RBF_INTERPOLATION:
    ...
    break;
  case _layer_xmapslab_ns::_subtype::SUBTYPE_LAB:
  ...
  Window->GL_widget->update_texture(Window->Map_name_to_layer[Name].Texture, Window->Map_name_to_layer[Name].Layer->image());

  Window->GL_widget->update();

  if (Window->state()==_window_xmapslab_ns::_state::STATE_STATISTICS){
#ifdef STATISTIC_GRAPHS_ACTIVE
    Window->update_chart(_window_xmapslab_ns::_mode_chart::MODE_CHART_ALL,Window->Statistics_x,Window->Statistics_y);
#endif
  }
}
