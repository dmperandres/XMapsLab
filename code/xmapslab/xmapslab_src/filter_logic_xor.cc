...
_filter_logic_xor::_filter_logic_xor()
{
  Filter_type=_filter_ns::_filter_type::FILTER_TYPE_LOGIC_XOR_OP;
...
void _filter_logic_xor::reset_data()
{
}

//HEA

float _filter_logic_xor::get_max_value()
{
...
//      if ((Value0>Threshold1 && Value1<=Threshold1) || (Value0<=Threshold1 && Value1>Threshold1)){
...
//  float Threshold1=float(Threshold)/100.0f;
  ...
  for (int Row=0;Row<Input_A->rows;Row++){
    for (int Col=0;Col<Input_A->cols;Col++){
      ...
      if ((Value0==1.0f && Value1==0.0f) || (Value0==0.0f && Value1==1.0f)){
...
_filter_logic_xor_ui::_filter_logic_xor_ui(_window_xmapslab *Window1, std::string Name1)
{
  Name=Name1;
  Qtw_filter_logic_xor=new _qtw_filter_logic_xor(Window1,this,Name1);
}

//HEA

_filter_logic_xor_ui::_filter_logic_xor_ui(_window_xmapslab *Window1, std::map<std::string, std::string> &Parameters, std::string Name1)
{
...
_filter_logic_xor_ui::~_filter_logic_xor_ui()
{
  delete Qtw_filter_logic_xor;
}

//HEA

void _filter_logic_xor_ui::reset_data()
{
  _filter_logic_xor::reset_data();
  Qtw_filter_logic_xor->parameter1(parameter1());
  hide();
}

//HEA

void _filter_logic_xor_ui::show()
{
  Qtw_filter_logic_xor->show();
}

//HEA

void _filter_logic_xor_ui::hide()
{
  Qtw_filter_logic_xor->hide();
}

//HEA

void *_filter_logic_xor_ui::get_link()
{
  return Qtw_filter_logic_xor->get_link();
}

//HEA

void _filter_logic_xor_ui::read_parameters(std::map<std::string,std::string> &Parameters)
{
  if (Parameters["_INI_"]=="EDITOR"){// default parameters
    parameter1(_f_logic_xor_ns::THRESHOLD_DEFAULT);
  }
  else{// Parameters from file or from initialised filter
    try{
      if (Parameters["threshold"]=="default") parameter1(_f_logic_xor_ns::THRESHOLD_DEFAULT);
...
void _filter_logic_xor_ui::write_parameters(std::map<std::string,std::string> &Parameters)
{
  char Aux[100];
...
//void _filter_logic_xor_ui::color(QColor Color1)
//{
//  Color=Color1;
//  Qtw_filter_logic_xor->parameter2(Color);
//}

//HEA

_qtw_filter_logic_xor::_qtw_filter_logic_xor(_window_xmapslab *Window1,_filter_logic_xor_ui *Filter1,std::string Box_name)
{
  Window=Window1;
  Filter=Filter1;
...
void _qtw_filter_logic_xor::parameter1(int Value)
{
...
//void _qtw_filter_logic_xor::parameter2(QColor Color1)
//{
...
void _qtw_filter_logic_xor::parameter1_slot(int Value)
{
...
//void _qtw_filter_logic_xor::parameter2_slot(QColor Color1)
//{
