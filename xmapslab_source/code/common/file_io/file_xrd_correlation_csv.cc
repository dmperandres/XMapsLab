...
void _file_xrd_correlation_csv::write(std::vector<std::string> &Vec_obs_names, std::vector<std::string> &Vec_pdb_names, std::vector<bool> &Vec_xrd_visible_columns_in_table, std::vector<std::vector<_data_xrd_ns::_data_xrd_obs_pdb> > &Mat_xrd_obs_vs_pdb_data)
{
  std::string Text;
  int R2;
  ...
  for (unsigned int Col=0;Col<Vec_xrd_visible_columns_in_table.size();Col++){
    if (Vec_xrd_visible_columns_in_table[Col]==true){
      File << ";" << Vec_pdb_names[Col];
    }
  }
  File << "\n";
  ...
  for (unsigned int Row=0;Row<Vec_obs_names.size();Row++){
    ...
    for (unsigned int Col=0;Col<Vec_xrd_visible_columns_in_table.size();Col++){
      if (Vec_xrd_visible_columns_in_table[Col]==true){
        // the correlations
        R2=int(roundf(powf(Mat_xrd_obs_vs_pdb_data[Row][Col].Correlation_coefficient,2)*100.0f));
