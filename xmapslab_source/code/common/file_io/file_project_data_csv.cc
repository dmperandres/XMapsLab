        ...
        if (It!=_file_project_data_csv_ns::Vec_allowed_words.end()){
          switch(It-_file_project_data_csv_ns::Vec_allowed_words.begin()){
            ...
            case int(_file_project_data_csv_ns::_allowed_tokens::TOKEN_TUBE):
              ...
              break;
            case int(_file_project_data_csv_ns::_allowed_tokens::TOKEN_XRF_DATA_ADJUSTMENT):
              if (Tokens[1]!=""){
                Text=Tokens[1];

                unsigned int i;
                for (i=0;i<_data_xrf_ns::Vec_xrf_data_adjustment_name.size();i++){
                  if (Text==_data_xrf_ns::Vec_xrf_data_adjustment_name[i].toStdString()) break;
                }

                if (i<_data_xrf_ns::Vec_xrf_data_adjustment_name.size()){
                  Project_data.XRF_data_adjustment=_data_xrf_ns::_xrf_data_adjustment(i);
                }
                else{
                  Error="The XRF_DATA_ADJUSTMENT value must be "+_data_xrf_ns::Vec_xrf_data_adjustment_name[0].toStdString();
                  for (unsigned int i=1;i<_data_xrf_ns::Vec_xrf_data_adjustment_name.size();i++) Error=Error+" or "+_data_xrf_ns::Vec_xrf_data_adjustment_name[i].toStdString();
                  return false;
                }
              }
              else{
                Error="There is not XRF_DATA_ADJUSTMENT value";
                return false;
              }
              break;
            case int(_file_project_data_csv_ns::_allowed_tokens::TOKEN_CS_ORIGIN):
              ...
              break;
            case int(_file_project_data_csv_ns::_allowed_tokens::TOKEN_WIDTH):
              if (Tokens[1]!=""){
                Text=Tokens[1];
                replace(Text.begin(),Text.end(),_common_ns::FROM_DECIMAL_SEPARATOR,_common_ns::TO_DECIMAL_SEPARATOR);
                Value=stof(Text);
                if (Value>=0) Project_data.Width_original=Value;
                else{
                  Error="The WIDTH is less than 0";
                  return false;
                }
              }
              else{
                Error="There is not WIDTH value";
                return false;
              }
              break;
            case int(_file_project_data_csv_ns::_allowed_tokens::TOKEN_HEIGHT):
              if (Tokens[1]!=""){
                Text=Tokens[1];
                replace(Text.begin(),Text.end(),_common_ns::FROM_DECIMAL_SEPARATOR,_common_ns::TO_DECIMAL_SEPARATOR);
                Value=stof(Text);
                if (Value>=0) Project_data.Height_original=Value;
                else{
                  Error="The HEIGHT is less than 0";
                  return false;
                }
              }
              else{
                Error="There is not HEIGHT value";
                return false;
              }
              break;
            case int(_file_project_data_csv_ns::_allowed_tokens::TOKEN_X):
              ...
              for (unsigned int i=0;i<Project_data.Vec_coordinate_x_original.size();i++){
                if (Tokens[i+2]!=""){
                  Text=Tokens[i+2];
                  Value=stoi(Text);
                  if (Value>=0 && Value<Project_data.Width_original) Project_data.Vec_coordinate_x_original[i]=Value;
                  else{
              ...
              if (Tokens.size()-2>=Project_data.Vec_coordinate_x_original.size()){
                ...
                for (unsigned int i=0;i<Project_data.Vec_coordinate_y_original.size();i++){
                  if (Tokens[i+2]!=""){
                    Text=Tokens[i+2];
                    Value=stoi(Text);
                    if (Value>=0 && Value<Project_data.Height_original) Project_data.Vec_coordinate_y_original[i]=Value;
                    else{
