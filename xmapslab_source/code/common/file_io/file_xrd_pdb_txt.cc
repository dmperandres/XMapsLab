...
bool _file_xrd_pdb_txt::read(std::vector<std::string> &Vec_names,std::vector<std::string> &Vec_formulas,std::vector<std::string> &Vec_elements,_vertex3f &Color,std::vector<float> &Vec_data, std::string &Error)
{
...
//    for (unsigned int i=0;i<Vec_data.size();i++) Vec_data[i]=Vec_data[i]/Max;
  }
...
//void _file_xrd_pdb_txt::write(vector<float> &Vec_data)
//{
