...
layout (location=3) uniform int Probe; // given the central position, this parameter indicates the number of pixels surounding it. This allows to get the mean value
layout (location=4) uniform float Diagonal; // this is the value of the diagonal of the hypercube. Used to normalize the final result
layout (location=5) uniform bool Normalization; // Position_normalization_type the normalization of data using the Diagonal value
layout (location=6) uniform int Position_normalization_type; // 0=Homogeneous (smArt_scan) 1=Heterogeneous
...
// the data of each measure (x,y,z). The position coordinates are NOT normalized
struct Data
{ 
  float Pos_x;          
  float Pos_y;
...
// the measures data
layout (binding=0,std430) buffer SSB_measure
{ 
  Data Vec_data[]; 
};
...
// the color of each measured position
layout (binding=2,std430) buffer SSB_measure_color
{ 
  vec4 Vec_color[];        
};
...
{ 
  ...
  for (int i=0;i<Num_samples;i++){ 		
		...
		if (Vec_valid_positions[i]==true){	
			...
			// computes the cartesian distance between the position of the treated pixel and the measured ones
			// X
...
void main(void)         
{
  // A vertex is emitted for each pixel. There are RowsxCols pixels. Linear to 2D convertion is applied
  // The column
  ...
  // The initial color is black=0 value
  ...
  // Check if the alpha value is 0 -> This position is no valid
  if (Color_normalized_input_image.a==0){
		// draw a chessboard patter using the module operator -> squares of size 10
    if (mod(Pos.x,20)<10 && mod(Pos.y,20)<10) Result_color=vec4(1,0,0,1); 
    else Result_color=vec4(0,0,0,1);        
  }          
  else{
	// It is normal position
