...
#ifndef LAYER_MAP_RBF_INTERPOLATION_H
#define LAYER_MAP_RBF_INTERPOLATION_H
...
#include "src/rbf/new_rbf_interpolation_2d.h"

#define DEFINED_LAYER_MAP_RBF_INTERPOLATION

namespace _layer_map_rbf_interpolation_ns
{
  // parameter 1
...
class _window;
class _qtw_layer_map_rbf_interpolation;

/*************************************************************************/

class _layer_map_rbf_interpolation: public _layer_map
{
public:
  _layer_map_rbf_interpolation();
  ~_layer_map_rbf_interpolation();

  void add_input_data(std::vector<float> &Vec_coordinate_x1, std::vector<float> &Vec_coordinate_y1, std::vector<float> &Vec_value1,float Epsilon1);
...
//  std::vector<_delaunay_triangulator_ns::_triangle> Triangles;
  ...
  std::vector<float> Interpolated_intensities;
...
class _layer_map_rbf_interpolation_ui : public _layer_map_rbf_interpolation
{
public:
  _layer_map_rbf_interpolation_ui(_window *Window1, std::string Name1=_layer_map_rbf_interpolation_ns::BOX_TEXT);
  ~_layer_map_rbf_interpolation_ui();
  void reset_data();
...
//  void get_parameters(std::map<std::string,std::string> &Parameters);
  ...
  void parameter1(int Value){_layer_map_rbf_interpolation::transparency(float(Value)/255.0f);}
  void parameter1_ui(float Value);
  int parameter1(){return int(_layer_map_rbf_interpolation::transparency()*255.0f);}

  void parameter2(int Value){_layer_map_rbf_interpolation::color_mixing_min(float(Value)/255.0f);}
  void parameter2_ui(float Value);
  int parameter2(){return int(_layer_map_rbf_interpolation::color_mixing_min()*255.0f);}

  void parameter3(int Value){_layer_map_rbf_interpolation::color_mixing_max(float(Value)/255.0f);}
  void parameter3_ui(float Value);
  int parameter3(){return int(_layer_map_rbf_interpolation::color_mixing_max()*255.0f);}

  void parameter4(int Value){_layer_map_rbf_interpolation::data_type_print(_palette_data_ns::_data_type_print(Value));}
  void parameter4_ui(int Value);
  int parameter4(){return int(_layer_map_rbf_interpolation::data_type_print());}
...
private:
  _qtw_layer_map_rbf_interpolation *Qtw_layer_map_rbf_interpolation_a=nullptr;
};

/*************************************************************************/

class _qtw_layer_map_rbf_interpolation: public QWidget
{
  Q_OBJECT
public:
  _qtw_layer_map_rbf_interpolation(_window *Window1,_layer_map_rbf_interpolation_ui *Filter1,std::string Box_name="Canny parameters");
...
//  QLineEdit *Line_edit_parameter1=nullptr;
  ...
  _layer_map_rbf_interpolation_ui *Filter=nullptr;
