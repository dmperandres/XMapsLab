...
#ifndef LAYER_POSITIONS
#define LAYER_POSITIONS
...
class _window;
class _qtw_layer_positions;

/*************************************************************************/

class _layer_positions : public _layer
{
public:
  _layer_positions();
  void reset_data();
...
class _layer_positions_ui : public _layer_positions
{
public:
  _layer_positions_ui(_window *Window1, std::string Name1=_layer_positions_ns::BOX_TEXT);
  ~_layer_positions_ui();
  void reset_data();
...
//  void get_parameters(std::map<std::string,std::string> &Parameters);
  ...
  void parameter1(int Value){_layer_positions::transparency(float(Value)/255.0f);}
  int parameter1(){return int(_layer_positions::transparency()*255.0f);}

  void parameter2(int Value){_layer_positions::pixel_transparency(bool(Value));}
  int parameter2(){return int(_layer_positions::pixel_transparency());}

private:
  _qtw_layer_positions *Qtw_layer_positions=nullptr;
};

/*************************************************************************/

class _qtw_layer_positions: public QWidget
{
  Q_OBJECT
public:
  _qtw_layer_positions(_window *Window1,_layer_positions_ui *Filter1,std::string Box_name="Canny parameters");
...
private:
  ...
  _layer_positions_ui *Filter=nullptr;
