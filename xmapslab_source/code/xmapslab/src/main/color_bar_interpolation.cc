...
_color_bar_interpolation::_color_bar_interpolation(_palette_data_ns::_color_type Color_type1, int Num_steps1, std::vector<float> &Vec_values1, std::vector<QColor> &Vec_colors1, _window *Window1):
...
void _color_bar_interpolation::mousePressEvent(QMouseEvent *Event)
{
  if (Changeable){
    ...
    if ((Pos_selected=row_at(Pos))>0 && Pos_selected<Num_steps){
  ...
  if (Changeable){
    if (Color_selected){
      ...
      if (std::abs(Diff.y())>1){
        ...
        emit colorChanged(Vec_proportions,Vec_colors,int(Data_type));
        Event->accept();
      }
    }
    else Event->ignore();
