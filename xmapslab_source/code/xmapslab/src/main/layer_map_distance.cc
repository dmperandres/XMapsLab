...
void _layer_map_distance::update_map()
{
  ...
  if (Computed==false){
    ...
    // diagonal
    glUniform1f(4,sqrt(Diagonal));

    // normalization
    glUniform1i(5,Normalization);

    // position normalization
    glUniform1i(6,int(Position_normalization_type));
...
_layer_map_distance_ui::_layer_map_distance_ui(_window *Window1, std::string Name1)
{
  Window=Window1;
  Name=Name1;
...
_qtw_layer_map_distance::_qtw_layer_map_distance(_window *Window1,_layer_map_distance_ui *Filter1,std::string Box_name)
{
  Window=Window1;
  Filter=Filter1;
