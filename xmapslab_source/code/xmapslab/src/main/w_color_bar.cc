...
_w_color_bar::_w_color_bar(_window *Window1): Window(Window1)
{
...
void _w_color_bar::create_palettes()
{
  ...
  for (unsigned int i=0;i<Window->Vec_palette_data.size();i++){
    if (Window->Vec_palette_data[i].data_type()==_palette_data_ns::_data_type::DATA_TYPE_DISCRETE){
      Palettes.push_back(new _color_bar_sections(Window->Vec_palette_data[i].color_type(),Window->Vec_palette_data[i].num_steps(),Window->Vec_palette_data[i].vec_values(),Window->Vec_palette_data[i].vec_colors(),Window));
    }
    else{
      Palettes.push_back(new _color_bar_interpolation(Window->Vec_palette_data[i].color_type(),Window->Vec_palette_data[i].num_steps(),Window->Vec_palette_data[i].vec_values(),Window->Vec_palette_data[i].vec_colors(),Window));
    }
...
void _w_color_bar::update_color_bar(_w_tab_ns::_tabs Selected_tab)
{
  switch(Selected_tab){
    case _w_tab_ns::_tabs::LAYERS_TAB:
      {
...
//          Widget_color_bar_palettes->hide();
    ...
    break;
  case _w_tab_ns::_tabs::TAB_PALETTES:
//    {
//      // palettes
...
//    }
    break;
  case _w_tab_ns::_tabs::POSITIONS_TAB:
  case _w_tab_ns::_tabs::COMPOUNDS_XRD_TAB:
  case _w_tab_ns::_tabs::COMPOUNDS_FROM_XRF_TAB:
  case _w_tab_ns::_tabs::ELEMENTS_TAB:
      // palettes
      Widget_color_bar_layers->hide();
...
_color_bar_abstract *_w_color_bar::create_color_bar(int Pos_palette, _pigment_data_ns::_data_type Data_type1, std::string Name_element1, float Max_value, std::vector<QColor> &Colors, _palette_data_ns::_data_type_print Data_type_print1, _color_map_ns::_color_for_zero_value Color_for_zero_value1)
{
  ...
  if (Data_type1==_pigment_data_ns::_data_type::DATA_TYPE_XRF) Color=Window->compute_color(Name_element1);
  else Color=Window->compute_color("");

  // it is necessary to compute the new colors depending on the element
  if (Palettes[Pos_palette]->type()==_palette_data_ns::_data_type::DATA_TYPE_DISCRETE){
    if (Palettes[Pos_palette]->color_type()==_palette_data_ns::_color_type::COLOR_TYPE_TONE){
      Color_map.compute_tones_sections(Palettes[Pos_palette]->values().size(),Colors,Color);
    }
    else{
      Color_map.compute_colors_sections(Palettes[Pos_palette]->values().size(),Colors,Color);
    }

    _color_bar_sections *Color_bar_sections=new _color_bar_sections(Palettes[Pos_palette]->color_type(),Palettes[Pos_palette]->num_steps(),Palettes[Pos_palette]->values(),Colors,Window);

    Color_bar=Color_bar_sections;
  }
  else{
    if (Palettes[Pos_palette]->color_type()==_palette_data_ns::_color_type::COLOR_TYPE_TONE){
      Color_map.compute_tones_interpolations(Palettes[Pos_palette]->values().size(),Colors,Color);
    }
    else{
      Color_map.compute_colors_interpolations(Palettes[Pos_palette]->values().size(),Colors,Color);
    }

    _color_bar_interpolation *Color_bar_interpolation=new _color_bar_interpolation(Palettes[Pos_palette]->color_type(),Palettes[Pos_palette]->num_steps(),Palettes[Pos_palette]->values(),Colors,Window);
...
//  cout << "change color" << endl;
  ...
  case _layer_ns::_layer_type::LAYER_TYPE_MAP_DISTANCE:
    std::dynamic_pointer_cast<_layer_map_distance_ui>(Window->Map_name_to_layer[Name].Layer)->set_colormap(Vec_values1,Vec_colors1,Type);
    break;
  case _layer_ns::_layer_type::LAYER_TYPE_MAP_TRIANGULATION:
    std::dynamic_pointer_cast<_layer_map_triangulation_ui>(Window->Map_name_to_layer[Name].Layer)->set_colormap(Vec_values1,Vec_colors1,Type);
    break;
  case _layer_ns::_layer_type::LAYER_TYPE_COMPOUND_COMBINATION:
    std::dynamic_pointer_cast<_layer_compound_combination_ui>(Window->Map_name_to_layer[Name].Layer)->set_colormap(Vec_values1,Vec_colors1,Type);
    break;
  case _layer_ns::_layer_type::LAYER_TYPE_MAP_RBF_INTERPOLATION:
    std::dynamic_pointer_cast<_layer_map_rbf_interpolation_ui>(Window->Map_name_to_layer[Name].Layer)->set_colormap(Vec_values1,Vec_colors1,Type);
    break;
  default:break;
  }
  ...
  if (Window->state()==_window_ns::_state::STATE_STATISTICS){
  }
}

/*************************************************************************/

void _w_color_bar::compute_regular_values_noise(int Num_values,std::vector<float> &Vec_values,float Noise)
{
  float t;
