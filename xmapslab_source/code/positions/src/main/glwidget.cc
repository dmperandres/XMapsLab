...
_gl_widget::_gl_widget(_window *Window1,int Position_gl_widget1):Window(Window1),Position_gl_widget(Position_gl_widget1)
{
...
void _gl_widget::update_texture(int Position_gl_widget1)
{
//  makeCurrent();
//  defaultFramebufferObject();

  GLuint Texture=Window->Textures[Position_gl_widget1];
  std::shared_ptr<cv::Mat> Image=Window->Layers[Position_gl_widget1]->image();
...
{
  if (Position_gl_widget==1 && Editing && Event->button()==Qt::RightButton){
...
void _gl_widget::draw_objects()
{
  ...
  if (Window->Layers.size()>0){
    ...
    // num textures
    glUniform1i(20,Window->Layers.size());
    // background color
    glUniform3fv(21,1,(GLfloat*) &COLORS[WHITE]);

    for (unsigned int i=0;i<Window->Layers.size();i++){
...
void _gl_widget::resizeGL(int Width1, int Height1)
{
  ...
  if (Position_gl_widget==1) Window->compute_dimensions();
}
